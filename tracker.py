...
def viewTickets():
	cTicket = tk.Tk()
	cTicket.geometry("1500x1000")
	cTicket.pack_propagate(False)
	cTicket.resizable(0, 0)

	def clear_data():
		tv1.delete(*tv1.get_children())

	cTicket.title("All Tickets")


	# Frame for Treeview
	frame1 = tk.LabelFrame(cTicket, text="Ticket Data")
...
# TODO Button(master, text='Close Ticket', command=closeTicket).grid(row=6, column=1, pady=4)
# TODO Button(master, text='Edit Ticket', command=editTicket).grid(row=6, column=1, pady=4)
tk.Button(master, text='Quit', command=master.quit).grid(row=6, column=2, pady=4)
